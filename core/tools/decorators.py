...
from core.tools.lib import send_user_dm, send_bot_embed, retrieve_application_emoji
    ...
    async def predicate(ctx):
        ...
        if guild_data:
            ...
            if ctx.channel.id not in guild_config.allowed_channels:
                await send_user_dm(ctx.author, description=":no_entry_sign: You can't use this command in this channel.")
                return False
        ...
        if user_data:
            ...
            if booster_command:
                if not ctx.author.premium_since:
                    await send_bot_embed(ctx, description=f"{await retrieve_application_emoji(emoji_name='booster', emoji_id='1297349785442979881')} You must be a server booster to claim this reward.")
                    return False
