...
from core.tools import send_bot_embed, economy_handler, color_autocomplete, coinflip_autocomplete
...
class BetCommands(Cog):
    ...
    @hybrid_command(name="roulette", aliases=["rl"], description="Bet on a color in roulette.")
    ...
    @hybrid_command(name="coinflip", aliases=["cf"], description="Bet on a side in a coinflip.")
    ...
    async def coinflip(self, ctx: Context, bet_amount, side: str) -> None:
        ...
        if bet_amount > MAX_COINFLIP:
            return await send_bot_embed(ctx, description=f":no_entry_sign: The maximum bet amount is **{MAX_COINFLIP}**.")
