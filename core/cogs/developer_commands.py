...
class DeveloperCommands(Cog):
    ...
    async def give_points(
        self, ctx: Context, amount: int, user: Optional[Member] = None
    ) -> None:
        """
        ...
        await send_bot_embed(
            ctx,
            title="Success",
            description=f":white_check_mark: You have given **{amount}** points to **{user.display_name}**.",
        )

    @command(
        name="donate", aliases=["give"], description="Donate money to another user."
    )
    ...
    async def donate(self, ctx: Context, user: Member, amount: int):
        ...
        if author_data.balance < amount and amount > 0:
            return await send_bot_embed(
                ctx, description=f"{paw_emoji} You don't have enough money to donate."
            )

        if not user_data:
            return await send_bot_embed(
                ctx,
                description=f"{paw_emoji} **{user.display_name}** is not registered.",
            )

        await update_user(user.id, balance=user_data.balance + amount)
        await update_user(ctx.author.id, balance=author_data.balance - amount)

        candy_emoji = await retrieve_application_emoji(
            "candy", 1295095109645373474, is_animated=True
        )
        await send_bot_embed(
            ctx,
            description=f"{candy_emoji} {ctx.author.display_name} has sucessfully donated **{amount}** candies to **{user.display_name}**.",
        )
    ...
    @command(
        ...
        aliases=["rc"],
        description="Register a channel for the bot to listen to.",
    )
    @admin_only()
    async def register_channel(self, ctx: Context) -> None:
        ...
        if not guild_config:
            ...
            return await send_bot_embed(
                ctx,
                description=":no_entry_sign: This guild is not registered. Try again.",
            )
        ...
        if guild_config.allowed_channels is None:
            ...
            return await send_bot_embed(
                ctx,
                description=":white_check_mark: This channel has been registered.",
                footer_text="This channel is now wishlisted for the bot to use.",
            )

        if ctx.channel.id in guild_config.allowed_channels:
            return await send_bot_embed(
                ctx, description=":no_entry_sign: This channel is already registered."
            )

        await update_guild(
            ctx.guild.id,
            allowed_channels=guild_config.allowed_channels + [ctx.channel.id],
        )
        await send_bot_embed(
            ctx,
            description=":white_check_mark: This channel has been registered.",
    ...
    @command(
        ...
        aliases=["urc"],
        description="Unregister a channel for the bot to listen to.",
    )
    @admin_only()
    async def unregister_channel(self, ctx: Context) -> None:
        ...
        if guild_config.allowed_channels is None:
            return await send_bot_embed(
                ctx, description=":no_entry_sign: This channel is not registered."
            )
        ...
        await send_bot_embed(
            ctx,
            description=":white_check_mark: This channel has been unregistered.",
            footer_text="This channel is no longer wishlisted for the bot to use.",
        )

    @command(
        name="registeritem",
        aliases=["ri"],
        description="Register an item in the bot's database.",
    )
    @admin_only()
    async def register_item(self, ctx: Context, item_id: int, item_price: int) -> None:
        ...
        if not item_creator_type == "Group" or item_creator_id != 6471663:
            return await send_bot_embed(
                ctx,
                description=":no_entry_sign: This item is not created by the UGC group.",
            )
        ...
        if not has_confirmed:
            return await send_bot_embed(
                ctx,
                description=":no_entry_sign: The registration process has been cancelled.",
            )

        await send_bot_embed(
            ctx,
            description=f":white_check_mark: The item has been successfully registered costing **{item_price}** candies.",
        )
        await create_item(
    ...
    async def parse_error_message(self, ctx: Context, item_info: dict):
        ...
        if code == 0:
            ...
            return await send_bot_embed(
                ctx, description=":no_entry_sign: The item ID you provided is invalid."
            )
        else:
            return await send_bot_embed(
                ctx,
    ...
    async def display_item(self, ctx: Context, item_id: int):
        ...
        if not item:
            return await send_bot_embed(
                ctx, description=":no_entry_sign: This item is not registered."
            )
        ...
        embed = await embed_builder(
            ...
            thumbnail=item_image,
            title="💻 Item Information",
            footer_text="Click any button to update the item.",
        )
        ...
        try:
            interaction = await self.bot.wait_for(
                "interaction", check=lambda i: i.user == ctx.author, timeout=60
            )
    ...
    async def button_handler(
        self, interaction: Interaction, interaction_data: dict, item_id: int
    ) -> None:
        """
    ...
    async def edit_buttons(self) -> tuple:
        ...
        """
        add_code = Button(
            style=ButtonStyle.green, label="Add code", emoji="➕", custom_id="add_code"
        )
        change_price = Button(
            style=ButtonStyle.blurple,
            label="Change price",
            emoji="💰",
            custom_id="change_price",
        )
        delete_item = Button(
    ...
    async def delete_item(self, interaction: Interaction, item_id: int) -> None:
        ...
        confirmation = await confirmation_popup(
            interaction, embed=embed, ephemeral=True
        )

        if not confirmation:
            return await interaction.followup.send(
                "❌ The deletion process has been cancelled.", ephemeral=True
            )
